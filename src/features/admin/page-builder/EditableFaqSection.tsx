        ...
        <div className={`mt-10 divide-y rounded-2xl border ${bg.isDark ? "divide-white/10 border-white/15" : "divide-herb/50 border-herb/50"}`}>
          {items.map((item, i) => (
            ...
            >
              ...
              <InlineText value={item.question} placeholder="Savol" onCommit={(v) => updateItem(i, { question: v })} className={`block pr-12 font-medium ${bg.heading}`} />
              <InlineText value={item.answer} placeholder="Javob" multiline onCommit={(v) => updateItem(i, { answer: v })} className={`mt-1.5 block text-sm ${bg.body}`} />
            </div>
          ))}
          <div className="px-6 py-4">
            <button
              type="button"
              onClick={addItem}
              className={`flex items-center gap-1.5 text-sm font-medium ${bg.isDark ? "text-white/60 hover:text-white" : "text-admin-muted hover:text-admin-primary"}`}
            >
